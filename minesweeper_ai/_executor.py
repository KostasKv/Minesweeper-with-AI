...
class _Executor:
    def __init__(self, game, num_games=None, game_seeds=None, seed=None):
        ...
        if num_games and game_seeds:
            raise ValueError(
                "Both num games and game seeds provided. Only one expected."
            )
    ...
    def make_move(self, action):
        ...
        is_in_end_game_state = self._game.state in [
            _Game.State.WIN,
            _Game.State.LOSE,
    ...
    def _is_legal_move(self, action):
        ...
        out_of_bounds = (
            (x < 0)
            or (y < 0)
            or (x >= self._game.config["columns"])
            or (y >= self._game.config["rows"])
        )
        ...
        while tiles_to_uncover:
            ...
            if tile.num_adjacent_mines == 0:
                ...
                for x, y in adjacent_tiles_coords:
                    if (
                        self._game.grid[y][x].uncovered == False
                        and not self._game.grid[y][x].is_flagged
                    ):
