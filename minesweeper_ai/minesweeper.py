...
def run(
    agent=None,
    config={"rows": 8, "columns": 8, "num_mines": 10, "first_click_is_zero": True},
    num_games=10,
    visualise=True,
    verbose=1,
    seed=None,
    ...
    if visualise:
        ...
        renderer = NoScreenRenderer(
            config, game.grid, agent, executor.current_game_seed
        )
...
def playGames(executor, renderer, verbose):
    ...
    while result:
        ...
        if verbose:
            print(
                "Made move {}.\tResult: {} mines left, game state {}".format(
    ...
    if _Game.is_end_of_game_state(state):
        ...
        if isinstance(renderer.agent, NoUnnecessaryGuessSolver):
            game_stats = {
                ...
                "first_click_always_zero": executor.get_game_config()[
                    "first_click_is_zero"
                ],
