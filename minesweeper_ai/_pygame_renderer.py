...
class PygameRenderer(Renderer):
        ...
        for y, row in enumerate(self.grid):
            ...
            for x, tile in enumerate(row):
                ...
                tile_sprite = TileSprite(
                    tile, self.game_state, pos, x, y, self.is_agent, self
                )
    ...
    def initialiseHUD(self):
        ...
        timer_x = (
            sprites["background"].get_width()
            - sprites["background_top_right"].get_width()
            - COUNTER_X_MARGIN
            - sprites["counter_backdrop"].get_width()
        )
        timer_y = COUNTER_Y
        timer_pos = (timer_x, timer_y)
        timer = Counter(timer_pos)

        emote_button_x = ((sprites["background"].get_width() // 2) - 3) - (
            self.TILE_SIZE // 2
        )
    ...
    def initialiseIllegalMoveTint(self):
        ...
        grid_size = (
            len(self.grid[0]) * self.TILE_SIZE,
            len(self.grid) * self.TILE_SIZE,
        )
    ...
    def initialiseAgent(self, game_seed):
        if self.agent:
            ...
            pygame.time.set_timer(
                self.PLAY_AGENT_MOVE_EVENT, self.AGENT_TIME_BETWEEN_MOVES
            )
    ...
    def update(self):
        if (
            not self.agent_next_move
            and self.agent
            and self.game_state in [_Game.State.PLAY, _Game.State.START]
        ):
    ...
    def onMouseButtonUp(self, event):
        ...
        if isinstance(sprite_at_cursor, TileSprite):
            ...
            isinstance(sprite_at_cursor, EmoteButton)
    ...
    def onGridClick(self, event, tile_sprite):
        ...
        if self.agent or (
            self.game_state
            in [_Game.State.WIN, _Game.State.LOSE, _Game.State.ILLEGAL_MOVE]
        ):
            return None
    ...
    def handleSpriteBeingHeldSwitch(self, event):
        ...
        if (
            isinstance(self.sprite_being_held, TileSprite)
            and self.sprite_being_held.holdable
        ):
    ...
    def getSpriteAtCursor(self):
        ...
        for sprite_group in self.things_to_draw:
            for sprite in sprite_group:
                ...
                ) and sprite.rect.collidepoint(cursor_pos):
                    return sprite
    ...
    def updateFromResult(self, result, game_seed):
        ...
        if self.game_state in [
            ...
            pygame.time.set_timer(
                self.CLOCK_TICK_EVENT, self.NO_TIMER
            )  # Stop the clock by disabling timer event
...
class TileSprite(pygame.sprite.Sprite):
    ...
    def isHoldable(self, tile, game_state, is_agent):
        return (
            not is_agent
            and not tile.uncovered
            and not tile.is_flagged
            and game_state in [_Game.State.PLAY, _Game.State.START]
        )
...
class BackgroundSprite(pygame.sprite.Sprite):
    ...
    def makeBackground(rows, columns, tile_size):
        width = (
            sprites["background_top_left"].get_width()
            + (tile_size * columns)
            + sprites["background_top_right"].get_width()
        )
        height = (
            sprites["background_top_left"].get_height()
            + (tile_size * rows)
            + sprites["background_bottom_left"].get_height()
        )
        ...
        background = BackgroundSprite.putOnBordersOfBackground(
            background, rows, columns
        )
        background = BackgroundSprite.putOnPlayAreaOfBackground(
            background, rows, columns
        )
    ...
    def putOnCornersOfBackground(background):
        ...
        top_right_x = (
            background.get_width() - sprites["background_top_right"].get_width()
        )
        bottom_left_y = (
            background.get_height() - sprites["background_bottom_left"].get_height()
        )
        bottom_right_x = (
            background.get_width() - sprites["background_bottom_right"].get_width()
        )
        bottom_right_y = (
            background.get_height() - sprites["background_bottom_right"].get_height()
        )
    ...
    def putOnBordersOfBackground(background, rows, columns):
        left_border = BackgroundSprite.scaleImageOnAxis(
            sprites["background_middle_left"], rows, scale_x_axis=False
        )
        right_border = BackgroundSprite.scaleImageOnAxis(
            sprites["background_middle_right"], rows, scale_x_axis=False
        )
        top_border = BackgroundSprite.scaleImageOnAxis(
            sprites["background_top_middle"], columns, scale_x_axis=True
        )
        bottom_border = BackgroundSprite.scaleImageOnAxis(
            sprites["background_bottom_middle"], columns, scale_x_axis=True
        )
    ...
    def putOnPlayAreaOfBackground(background, rows, columns):
        play_area_scaled_y = BackgroundSprite.scaleImageOnAxis(
            sprites["background_middle"], rows, scale_x_axis=False
        )
        play_area = BackgroundSprite.scaleImageOnAxis(
            play_area_scaled_y, columns, scale_x_axis=True
        )
