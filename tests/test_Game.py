...
class TestGameConfigurationValidation(unittest.TestCase):
        ...
        for config in valid_configurations:
            with self.subTest():
                try:
                    ...
                    self.fail("Incorrectly raised exception on valid configuration {}".format(config))
    ...
    def test_raiseErrorOnInvalidConfig_invalid_configurations(self):
        ...
        for config in invalid_configurations:
            with self.subTest():
                with self.assertRaises((TypeError, ValueError, KeyError), msg="Failed on invalid config: {}".format(config)):
                    Game.raiseErrorOnInvalidConfig(config)
...
class TestGameMethods(unittest.TestCase):
    ...
    def testSeededGameFixtureIsWhatIsExpected(self):
        ...
        grid_is_as_expected = self.isGridEqualToExpectedGridValuesAndAllTilesAreCovered(self.game.grid, expected_grid_values)
    ...
    def test_generateNewGrid(self):
        ...
        new_grid_is_as_expected = self.isGridEqualToExpectedGridValuesAndAllTilesAreCovered(self.game.grid, expected_new_grid)
        self.assertTrue(new_grid_is_as_expected)

    @staticmethod
    def isGridEqualToExpectedGridValuesAndAllTilesAreCovered(grid, expected_grid_values):
        rows = len(grid)
        columns = len(grid[0])
