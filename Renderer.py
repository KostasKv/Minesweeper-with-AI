...
class PygameRenderer:
    def __init__(self, executor):
        ...
        self.game_config = executor.getGameConfig()
        self.grid = executor.getGrid()
        ...
        self.TILE_SIZE = 16
        self.TIMER_TICK = pygame.USEREVENT + 1
    ...
    def initialiseGrid(self):
        ...
        pos = (grid_x, grid_y)
        # self.grid_rect =  pygame.Rect(grid_x, grid_y, width, height)
        # grid_obj = {'name': 'grid', 'obj': self.grid_rect}

        grid_obj = Grid(self.screen, self.sprites, pos, self.TILE_SIZE, self.grid, self.game_state)
    ...
    def initialiseHUD(self):
        ...
        emote_button = EmoteButton(self.screen, self.sprites, emote_button_pos, self.game_state)
    ...
    def handleEvents(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if isinstance(object_at_cursor, Grid) and not (self.game_state.value == State.WON.value or self.game_state.value == State.LOST.value):          
                    if self.game_state == State.START:
                        # Event reoccurs every second (allowing game time to be counted)
                        pygame.time.set_timer(self.TIMER_TICK, 1000) 
                    

                    (x, y) = object_at_cursor.getCellCoordinatesAtPositionOnScreen(pos)
                    flag_cell = event.button == MouseButton.RIGHT.value

                    # SEND ACTION TO EXECUTOR
                    result = self.executor.makeMove(x, y, flag_cell)

                    self.updateFromResult(result, x, y)
    ...
    def drawCell(self, cell, cell_top_left_pos):
        if cell.uncovered:
            if cell.is_mine:
                tile = self.sprites['tile_mine']
            else:
                sprite_name = self.TILE_NUM_TO_SPRITE_NAME[cell.num_adjacent_mines]
                tile = self.sprites[sprite_name]
        else:
            if cell.is_flagged:
                tile = self.sprites['tile_flag']
            else:
                tile = self.sprites['tile_covered']
        
        self.screen.blit(tile, cell_top_left_pos)


    def updateFromResult(self, result, x, y):
        self.grid = result[0]
        self.game_state = result[1]

        if self.game_state.value in [State.LOST.value, State.WON.value]:
            pygame.time.set_timer(self.TIMER_TICK, 0)    # Stop the clock by disabling timer event

        if self.game_state.value == State.LOST.value:
            for obj in self.rendered_objects:
                if isinstance(obj, Grid):
                    obj.mine_clicked_coords = (x, y)
...
class Counter():
    ...
    def draw(self):
        ...
        padded_digits_string = str(self.value).zfill(self.NUM_DIGITS)
...
class EmoteButton():
    def __init__(self, screen, sprites, pos, game_state):
        self.screen = screen
        self.sprites = sprites
        self.rect = self.initialiseRect(pos)
    ...
    def getEmote(self):
        ...
        elif self.game_state.value == State.WON.value:
            sprite = self.sprites['emote_cool']
        elif self.game_state.value == State.LOST.value:
            sprite = self.sprites['emote_dead']
        elif self.object_being_held == None: 
            sprite = self.sprites['emote_smile']
        elif isinstance(self.object_being_held, Grid):
...
class Grid():
    def __init__(self, screen, sprites, pos, tile_size, grid, game_state):
        self.screen = screen
        self.sprites = sprites
    ...
    def getCellSprite(self, cell, x, y):
        if cell.uncovered:
            ...
            elif self.cell_being_held_coordinates == (x, y) and not self.game_state.value in [State.WON.value, State.LOST.value]:
                sprite = self.sprites['tile_uncovered'] 
            else:
                sprite = self.sprites['tile_covered']
        
        # End of game
        if self.game_state.value == State.WON.value:
            if cell.is_mine:
                sprite = self.sprites['tile_flag']
        elif self.game_state.value == State.LOST.value:
            if cell.is_mine:
